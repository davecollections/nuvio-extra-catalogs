...
Complete the Issue #33 BAFTA source and lineage audit before canonical import. Use rendered first-party BAFTA Film, Television, and Television Craft histories to produce a committed winner-only snapshot, document every current-lineage inclusion/exclusion, and prove movie/series classification can be reproduced offline. `Xtra` remains a working identity; do not rename the repository, GitHub Pages path, manifest routes, or stable add-on/catalogue IDs until the owner locks and explicitly approves the final brand and migration plan.
