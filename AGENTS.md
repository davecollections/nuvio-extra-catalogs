...
V1.3 BAFTA Television is complete and owner-accepted. PR #38 merged as `860a79cff358393137b4a28ad25d60b5a1faf2a1`; main CI and GitHub Pages passed, and all 367 deployed landing-page, manifest, preset, catalogue, and metadata files byte-match that accepted merge. All 27 Television lineages produce 45 deterministic catalogues from 931 canonical winner records and 970 work links after seven exact duplicate source relationships are collapsed. All 888 scoped Television identity units are reviewed: 864 resolve to compatible IMDb works and 24 are explicit non-catalogue outcomes. Live artwork review covers all 667 published titles: 563 use MetaHub, eight use verified TMDB fallbacks, and 96 are explicit unavailable-poster outcomes. Live metadata review shows 616 resolve through Nuvio's recommended provider and 51 require exact static fallbacks; Cinemeta resolves 39 of those gaps and 12 remain unresolved by either provider. Owner acceptance confirmed movie and series fallback detail routes plus an unaffected full provider result in Nuvio. The all-awards manifest is `1.3.0` with 129 catalogues and a distinct BAFTA Television preset. Known-good milestones are preserved by annotated semantic-version tags and GitHub Releases rather than permanent release branches. BAFTA Television Craft is the V1.4 implementation target; preserve its reviewed snapshot, lineage decisions, and identity evidence. `Xtra` remains a working identity: do not rename the repository, GitHub Pages path, or other stable routes until the owner explicitly approves the final brand.
...
Implement BAFTA Television Craft as V1.4 using the already reviewed source snapshot, lineage decisions, and identity evidence. Keep its catalogue, metadata, artwork, preset, and Nuvio acceptance work independently testable before publication. `Xtra` remains a working identity; do not rename the repository, GitHub Pages path, manifest routes, or stable add-on/catalogue IDs until the owner locks and explicitly approves the final brand and migration plan.
