...
Complete the Issue #33 BAFTA lineage audit before canonical import. Map or explicitly exclude all 291 pinned historical labels against the 75 current work-associated categories, preserve gaps and discontinued categories without guesswork, and prove movie/series classification can be reproduced offline. `Xtra` remains a working identity; do not rename the repository, GitHub Pages path, manifest routes, or stable add-on/catalogue IDs until the owner locks and explicitly approves the final brand and migration plan.
